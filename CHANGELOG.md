...
- After a published audit of a site on a recurring schedule, the CLI prints one line naming the cadence and the link that turns it off (#372). `squirrel report --publish` keeps a bare URL on stdout and prints the line on stderr. The line renders what the cloud sends, so it appears once the cloud side ships.
