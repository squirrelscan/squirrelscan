...
- Auto-update now keeps your agent skills current too. On the first run after the CLI updates itself, a background job refreshes the squirrelscan skills you installed globally with `npx skills`. It re-adds an install recorded from `squirrelscan/squirrelscan` from squirrelscan/skills and updates the rest. It only touches skills you already have, never prompts, runs once per version, stops after 60 seconds, skips without `npx`, and follows the auto-update opt-outs (`auto_update`, `SQUIRREL_NO_UPDATE`, CI). It runs after the update has landed, so it can never fail or delay it. `squirrel skills` now runs the skills CLI as `skills@1`, the major its commands were checked against.
