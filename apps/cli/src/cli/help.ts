...
function section(t: Theme, title: string): string {
  const rule = t.dim(
    t.sym.rule.repeat(Math.max(3, RULE_WIDTH - title.length - 1))
  );
...
function rows(
  ...
  return items.map(
    ([name, desc]) => `${" ".repeat(indent)}${padVisible(name, width)}${desc}`
  );
}
...
function groupLines(t: Theme): string[] {
  const width =
    Math.max(
      ...COMMAND_GROUPS.flatMap((g) => g.commands.map(([n]) => n.length))
    ) + 3;
  const out: string[] = [];
  for (const group of COMMAND_GROUPS) {
...
export function renderHome(
  t: Theme,
  { version, setupDone }: HomeOptions
): string {
  const quick: Array<[string, string]> = [];
  if (!setupDone)
    quick.push([
      t.command("squirrel setup"),
      "Sign in, add the agent skills, set defaults",
    ]);
  quick.push(
    [
      t.command("squirrel audit https://example.com"),
      "Audit a website (free, runs locally)",
    ],
    [t.command("squirrel report"), "Look at your latest report"]
  );
  const lines = ["", t.header({ version: `v${version}` }), ""];
  lines.push(
    section(t, "Quick start"),
    ...rows(quick),
    ...groupLines(t),
    ...footer(t)
  );
...
export function renderRootHelp(t: Theme, version: string): string {
  const options: Array<[string, string]> = [
    [
      t.command("-c, --config-file <path>"),
      "Use this squirrel.toml instead of the nearest one",
    ],
...
async function resolveArgs(cmd: AnyCommand): Promise<ArgsDef> {
  const args =
    typeof cmd.args === "function" ? await cmd.args() : await cmd.args;
...
async function resolveSubCommands(
  ...
  for (const [name, sub] of Object.entries(subs ?? {})) {
    out[name] = (
      typeof sub === "function"
        ? await (sub as () => Promise<AnyCommand>)()
        : await sub
    ) as AnyCommand;
  }
  return out;
...
async function describeArgs(t: Theme, cmd: AnyCommand) {
  ...
  for (const [name, raw] of Object.entries(args)) {
    ...
    const def =
      arg.default !== undefined && arg.type === "string"
        ? t.dim(` (default: ${String(arg.default)})`)
        : "";
...
export async function renderCommandHelp(
  ...
  const description =
    (path.length === 1 ? describeTopLevel(path[0]!) : undefined) ??
    meta.description ??
    "";
  ...
  if (Object.keys(subs).length > 0) {
    lines.push(
      `${t.bold("Usage")}  ${full} ${t.dim("<command> [options]")}`,
      "",
      section(t, "Commands")
    );
    ...
    lines.push(...rows(items));
    if (options.length > 0)
      lines.push("", section(t, "Options"), ...rows(options));
    lines.push(
      "",
      `Run ${t.command(`${full} <command> --help`)} for a command's options.`,
      ""
    );
    return lines.join("\n");
  }

  const usageTail = [...usage, options.length > 0 ? "[options]" : ""]
    .filter(Boolean)
    .join(" ");
  lines.push(`${t.bold("Usage")}  ${full} ${t.dim(usageTail)}`);
  if (positionals.length > 0)
    lines.push("", section(t, "Arguments"), ...rows(positionals));
  if (options.length > 0)
    lines.push("", section(t, "Options"), ...rows(options));
...
export async function renderUnknownCommand(
  ...
  const subs =
    typeof parent.subCommands === "function"
      ? await parent.subCommands()
      : await parent.subCommands;
...
export async function renderMissingArgument(
  ...
  const what = message
    .replace(/^Missing required (positional )?argument:\s*/i, "")
    .trim();
  const noun = /^(url|id|api)$/i.test(what)
    ? what.toUpperCase()
    : what.toLowerCase();
  const wanted = what.startsWith("--")
    ? `the ${what} option`
    : `${/^[aeiou]/i.test(noun) && noun !== "URL" ? "an" : "a"} ${noun}`;
  const usageTail = [...usage, options.length > 0 ? "[options]" : ""]
    .filter(Boolean)
    .join(" ");
  const lines = [
    "",
    `${t.error(t.sym.error)} ${t.bold(`${full} needs ${wanted}.`)}`,
    "",
    `  ${t.bold("Usage")}    ${full} ${t.dim(usageTail)}`,
  ];
