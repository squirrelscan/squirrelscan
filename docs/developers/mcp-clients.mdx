---
title: "Connect Claude Code, Cursor, or Codex to website audit MCP"
sidebarTitle: "MCP client setup"
description: "Copy-paste config that connects Claude Code, Cursor, Codex CLI, VS Code, Windsurf, or Gemini CLI to squirrelscan's hosted website audit MCP server."
icon: "toy-brick"
---
...
- **OAuth (recommended for a human at an editor).** Point the client at the URL with no auth headers. It discovers the authorization server, registers itself, and runs a browser login. Nothing secret lands in a config file.
...
Then, inside a session, run `/mcp`, select **squirrelscan**, and choose **Authenticate**. Your browser opens so you can log in and org selection. From the shell without a session, `claude mcp login squirrelscan` (Claude Code v2.1.186+) does the same; add `--no-browser` for headless or SSH (v2.1.191+). Clear tokens with `claude mcp logout squirrelscan`.
...
<Tabs>
  <Tab title="OAuth (recommended)">
    ...
    On first connection VS Code discovers the authorization server, registers a client, and opens a browser so you can log in. Manage the grant under **Accounts → Manage Trusted MCP Servers**.
  </Tab>
  <Tab title="API key">
