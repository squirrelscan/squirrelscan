...
<Tip>Windsurf does not implement the [Agent Skills](https://agentskills.io) standard, so `squirrel skills install` has no effect here, unlike Cursor or Claude Code. The MCP tools and the CLI are the two supported paths.</Tip>
