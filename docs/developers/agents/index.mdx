...
<CardGroup cols={3}>
  ...
  <Card title="Skills" icon="sparkles">
    Autonomous audit + fix workflows the agent discovers and runs on its own. Install once: `squirrel skills install`.
  </Card>
...
The [Agent Skills](https://agentskills.io) standard ships two squirrelscan skills. They install the same way for every agent (`squirrel skills install`, landing in `~/.agents/skills/` and `~/.claude/skills/`); Claude Code can also get them bundled in the [plugin](/developers/agents/claude-code).
