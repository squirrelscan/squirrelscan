...
<CardGroup cols={3}>
  ...
  <Card title="Skills" icon="sparkles">
    Autonomous audit + fix workflows the agent discovers and runs on its own. Install once: `npx skills add squirrelscan/skills`.
  </Card>
...
The [Agent Skills](https://agentskills.io) standard ships two squirrelscan skills. They install the same way for every agent (`npx skills add squirrelscan/skills`, landing in `.agents/skills/`); Claude Code can also get them bundled in the [plugin](/developers/agents/claude-code).
...
See the [configuration reference](/configuration) for all options, and [Fix your website with a coding agent](/guides/fix-your-site-with-an-ai-agent) for a full worked example.
