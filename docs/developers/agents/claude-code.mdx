...
<Tabs>
  <Tab title="Plugin (recommended)">
    ...
    Added the older `squirrelscan/squirrelscan` marketplace before? It serves the same skills and keeps working. Both marketplaces are named `squirrelscan`, so to switch, run `/plugin marketplace remove squirrelscan` first.
  </Tab>
  <Tab title="MCP only">
