...
<Tabs>
  <Tab title="Plugin (recommended)">
    ...
    Added the older `squirrelscan/squirrelscan` marketplace before? It now points the plugin at squirrelscan/skills, so it keeps working: run `/plugin marketplace update squirrelscan` then `/plugin update squirrelscan@squirrelscan`. To switch marketplaces instead, remove the old one first (Claude Code won't add a second marketplace named `squirrelscan`, and removing it uninstalls the plugin):
...
Local audits need nothing. For hosted crawling, publishing, and credits, log in once. With the MCP server added, run `/mcp` → **squirrelscan** → **Authenticate**, or from the shell:
