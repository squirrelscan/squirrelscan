...
token. If you would rather not manage a key, its OAuth login mints a scoped,
...
If your account belongs to more than one organization, name the one you mean:
`squirrel keys create --org <slug|id>`. Without it the CLI lists your
organizations and stops rather than minting against one you did not choose. See
[`squirrel keys`](/cli/keys#keys-belong-to-one-organization).
