...
title: "Quickstart"
icon: "play"
description: "Install squirrelscan and run your first audit"
---
...
Then run setup once. It signs you in (optional), installs the agent skills for Claude Code, Codex, Cursor and more, and turns on auto-update:

```bash
squirrel setup
```

<Tip>Run `squirrel` on its own any time to see every command and a quick start. If something looks wrong, `squirrel self doctor` checks the install.</Tip>
...
| --- | --- |
| `SQUIRREL_VERSION` | Install a specific release instead of the newest one, for example `v0.0.94`. |
| `SQUIRREL_CHANNEL` | `stable` (default) or `beta`. |
| `SQUIRREL_BIN_DIR` | Where to put the `squirrel` symlink. Defaults to the first writable directory on your PATH, falling back to `~/.local/bin`. |
| `SQUIRREL_FORCE_MIRROR` | Download from squirrelscan's mirror instead of GitHub. See below. |
| `NO_TELEMETRY` | Any non-empty value turns off the anonymous failure report the installer sends when it fails. |
