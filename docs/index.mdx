...
<CardGroup cols={2}>
  ...
  >
    Use squirrelscan with Claude Code, Cursor, and other coding agents.
  </Card>
</CardGroup>
...
<Columns cols={2}>
  ...
  <Card
    title="Crawl history and changes"
...
squirrelscan runs **295 rules** across **21 categories**, plus opt-in cloud gap analysis. Ordered by how much a failure usually costs you, not by how many rules each one has.
...
| **Structured Data** | 25 | JSON-LD validity and rich-result eligibility for Article, Product, FAQ, Review, Breadcrumb, Organization, LocalBusiness, Video and site search, plus thirteen site-wide checks on the entity map: whether the site declares one identity for itself or a new one on every page, plus pages missing the markup their same-type siblings have and rating markup that is not about the page it sits on |
...
**Total: 295 rules across 21 categories.** Gap analysis (keyword and content gaps) is a cloud opt-in and is not counted above.
