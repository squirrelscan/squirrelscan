...
This adds all the squirrelscan skills, currently `squirrelscan` (operate the CLI: login, publish reports, MCP, credits) and `audit-website` (the audit-and-fix loop). It runs `npx skills add` under the hood. See [`squirrel skills`](/cli/skills) and the [Coding agent integration](/developers/agents) guide for what the skills do.
