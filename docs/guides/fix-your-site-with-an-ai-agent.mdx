---
title: "Fix your website with a coding agent"
...
This adds all the squirrelscan skills, currently `squirrelscan` (operate the CLI: login, publish reports, MCP, credits) and `audit-website` (the audit-and-fix loop). squirrel downloads and verifies them itself, and its auto-update keeps them current. See [`squirrel skills`](/cli/skills) and the [Coding agent integration](/developers/agents) guide for what the skills do.
...
<Tabs>
  <Tab title="Claude Code">
    ...
    Start `claude` in the project, run `/mcp`, select **squirrelscan**, and choose **Authenticate**. Your browser opens so you can log in and organization selection.
  </Tab>
  <Tab title="Codex CLI">
    ```bash
    ...
    `codex mcp login` runs the browser login and stores the token encrypted.
  </Tab>
  <Tab title="Cursor">
...
- [Coding agent integration](/developers/agents) - skills, plan mode, and subagent patterns
