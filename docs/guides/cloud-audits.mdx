...
Log in once so your audits can reach the cloud:

```bash
squirrel auth login
```

This opens your browser so you can log in. What runs next depends on the audit's coverage mode. Once you are logged in (free or Pro), audits default to `surface` coverage, which runs the cloud rules and merges their results into the same report as the local rules, spending credits as they run. Only anonymous, logged-out runs default to `quick`. Want a fast, free scan while logged in? Pass `-C quick`. Free accounts get 500 credits a month to spend; Pro includes 3,000. See [`squirrel auth`](/cli/auth) for API-key auth and credential precedence, and [`squirrel audit`](/cli/audit#coverage-modes) for the coverage modes.
...
<Tabs>
  <Tab title="CLI">
    Run an audit with the cloud rules included. Logged in, the default `surface` coverage already runs them (and, when relevant, pages render in a cloud browser); the flag below just makes it explicit:
  ...
  <Tab title="MCP">
    Your agent runs the audit as a tool call. Point any MCP client at the [hosted MCP server](/developers/mcp) and it calls `run_audit` with the URL, confirms the credit estimate, then polls `get_audit_status` and reads the report. The full agent workflow is in [Fix your website with a coding agent](/guides/fix-your-site-with-an-ai-agent).
  </Tab>
</Tabs>
...
When you are logged in and online, every audit auto-publishes to your dashboard as an **unlisted** report, giving you a shareable link plus issue history with no extra flags. Publishing is included in the audit base charge, whatever the visibility.
...
- [Fix your website with a coding agent](/guides/fix-your-site-with-an-ai-agent) - the full agent fix loop
