...
Default [coverage mode](/crawl) for crawls and audits. When unset, `audit` picks the mode by auth state (`surface` logged in, `quick` anonymous) while `crawl` always uses `quick`. Each mode sets the default page budget (`quick` 25, `surface` 100, `full` 500) and controls link discovery and cloud enrichment.
...
How long the crawl will wait out a host that is rate limiting it. The value caps
both a single backoff wait and the total waiting spent on one URL or host.

When a host answers `429 Too Many Requests` (or Shopify's `430`, or a `503` that
carries a `Retry-After` header), squirrelscan does not simply fail the page:

- **Every request to that host pauses** for the backoff window, not just the one
  that was refused. Requests to other hosts are unaffected.
- The host drops to **one in-flight request**, and its `per_host_delay_ms` is
  multiplied while it recovers.
- `Retry-After` is honoured as the minimum wait, in both its seconds and
  HTTP-date forms, clamped to `max_backoff_ms`.
- Without `Retry-After` the first wait is 5 seconds, doubling with jitter on each
  further refusal, for up to six attempts.
- After a sustained run of successful responses the host earns its configured
  concurrency and delay back in steps.

Once a host has been throttled for longer than `max_backoff_ms` without
recovering, its remaining URLs are recorded as **rate limited** rather than
fetched, and the crawl finishes with the other hosts. Rate-limited URLs are never
reported as broken links or 4xx pages: their status is unknown, so they are
listed in an informational finding and excluded from scoring.

**Examples:**

Give a slow host longer to recover:
```toml
[crawler]
max_backoff_ms = 600000  # 10 minutes
```

Fail fast in CI, where a long wait is worse than a short report:
```toml
[crawler]
max_backoff_ms = 30000  # 30 seconds
```

See [Crawling rate-limited hosts](/guides/rate-limited-hosts) for the full
recipe, including Shopify storefronts.
...
`*` is the only special character, and it stands for any run of characters,
`/` included. There is no `?` or `[abc]`: those match themselves.

A pattern is matched against the URL's **path** unless it starts with `http://`
or `https://`, in which case it is matched against the whole URL. So a pattern
that needs to see a query string or a host has to be absolute.
...
| `*.html` | the whole path, anchored and case-insensitive |
| `https://example.com/*?preview=*` | the whole URL, anchored and case-insensitive |

A pattern ending in `/*` is a plain prefix test, which is both the cheapest form
and the one to reach for. `**` is not a separate construct: it is two `*` in a
row and matches exactly what one `*` does, so older examples written as
`/blog/**` still work.
...
<Note>
  Cloud audits don't read this file. The same two lists live on the dashboard at
  **Website → Settings → Crawl**, and apply to every cloud audit of that site.
  They behave slightly differently there: a cloud `include` pattern is pinned to
  the website's own addresses before the crawl runs, so it can never widen the
  crawl onto another host. See [Crawl settings](/dashboard/crawl-settings).
</Note>
...
Exclude query parameters. These have to be absolute, because a relative
pattern is matched against the path, which never contains a query string:
```toml
[crawler]
exclude = ["https://example.com/*?preview=*", "https://example.com/*?draft=*"]
```
...
|---------|---------|---------------|
| `/blog/*` | `/blog/post`, `/blog/post/comment` | `/about`, `/blog` |
| `/blog` | `/blog`, `/blog/post`, `/blogroll` | `/about` |
| `*.pdf` | `/file.pdf`, `/docs/guide.pdf` | `/file.html` |
| `/api/*/users` | `/api/v1/users`, `/api/v1/v2/users` | `/api/users` |
| `https://example.com/*?preview=*` | `https://example.com/page?preview=true` | `/page?preview=true` on another host |

`*` does not stop at a `/`, so a pattern cannot select one level of a path and
not the level below it. Reach for `exclude` when you need to carve a subtree
back out.
...
<Note>
  A custom header configured for a website's **cloud** audits does more than
  authenticate the request: it marks the crawl as authorised, which raises the
  crawl rate and stops robots.txt being applied at all. See
  [Authorised crawls](/dashboard/crawl-settings#authorised-crawls).
</Note>
