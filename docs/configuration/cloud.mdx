...
publish = true                # auto-publish to your dashboard when logged in
...
Auto-publish the audit report to your [dashboard](/dashboard) when you're **logged in and online** - independent of cloud rendering or enrichment. **Publishing is included in the audit base charge**, whatever the visibility. Logged-out and `--offline` runs never publish. Set to `false` to never auto-publish for this project; you can still publish a single run with `--publish` (it overrides this). Skip publishing one run with `--no-publish`.
