...
Wherever that warning appears, it states what the run would have cost, the credits you have left, the date your monthly credits reset, and an upgrade link. The link is scoped to the organization that ran out, so it opens checkout for that organization rather than whichever one your browser used last. Credits are not shared between organizations, so that distinction matters: topping up the wrong one leaves the blocked audit blocked. The MCP server returns the same four facts as fields on its `run_audit` response, so an agent can relay them without a second call.
