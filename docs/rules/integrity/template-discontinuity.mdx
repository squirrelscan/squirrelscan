...
- Skipped when the crawl produced fewer pages than `minPages`. Message: `Need >=N pages to establish a template baseline`.
...
- `template-discontinuity` warns for below-threshold pages that carry at least one page-level integrity signal **and** load nothing the rest of the site loads. Message: `N off-template page(s) share none of the site's assets and carry a page-level integrity signal`, listing up to five as `/path (sim S)`.
- A below-threshold page is never escalated when it loads one of the site's own stylesheets, or loads a stylesheet or a script from a host that serves a stylesheet, script or image on a majority of the crawled pages. An injected standalone page brings its own everything; a signed-out account view pulling the site's CSS bundle is the site's own page, however little of the theme it renders.
- Three things deliberately do not count as the site's assets. A `link` that is not a stylesheet, so a `canonical`, an `icon`, a `preconnect`, a `dns-prefetch` or an `alternate` pointing at the site proves nothing about who rendered the page. A shared public host: `fonts.googleapis.com`, `fonts.gstatic.com`, `ajax.googleapis.com`, `cdnjs.cloudflare.com`, and any subdomain of `jsdelivr.net` or `unpkg.com`, since every site on the web can load those. And an image on its own, because a standalone page can hotlink a logo.
- Nothing is escalated when the crawl stopped at its page limit and the baseline came from fewer than `minBaselinePagesWhenCapped` pages. A capped crawl saw the first N pages of the site, not the site, so its baseline can be one section's template and everything outside that section looks foreign. Those pages are still reported for review, and the review check's details carry `escalationWithheld: capped_crawl_small_baseline`. Raising the page limit is what changes the verdict.
- `template-discontinuity-review` reports info for below-threshold pages that were not escalated. Message: `N page(s) diverge from the site template (review)`.

Severity is warning, weight 6.

## How to fix it

Open the listed pages and compare them with a normal page from the same site. Most of the time the answer is that you published it: a campaign landing page, a signed-out or empty state, a checkout step. If a page is yours, load the shared stylesheet and the site chrome on it, which also fixes the inconsistency a visitor would notice and stops the page being reported. Only if you did not publish it does the compromise reading apply, and then remove it, audit recently modified files and read the access logs.
...
| `minBaselinePagesWhenCapped` | integer, 0 or more | `20` | When the crawl stopped at its page limit, escalate only if the baseline was built from at least this many pages; below it, outliers are still reported for review |
