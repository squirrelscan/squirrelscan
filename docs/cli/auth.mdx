...
Authenticated as you@example.com
  ...
  Org: acme (Acme Inc)
  Org id: org_2xK9vQ
  Active org of 2. Cloud runs spend this org's credits; pass --org to target another.
  Device: macbook-pro
  Expires: Feb 2, 2027
  Credits: 950
```

`Org` shows the slug (what `--org` takes, and what your dashboard URL uses);
`Org id` shows the id that appears on credit ledger rows. The count line appears
only when you belong to more than one org. Resolving the org needs the API, so
the org lines are omitted when it can't be reached: `status` still reports your
identity.
