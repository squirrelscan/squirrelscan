...
The running process is never interrupted - the new version takes effect on the next run. From v0.0.99, that run also refreshes any squirrelscan agent skills you installed globally with `npx skills` or `squirrel skills install`, in the background (see [skills](/cli/skills#automatic-updates)). Turn this off with:
