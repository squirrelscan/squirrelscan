...
The running process is never interrupted - the new version takes effect on the next run. From v0.0.99, that run also refreshes the squirrelscan agent skills you have installed, in the background (see [skills](/cli/skills#automatic-updates); turn just that off with `squirrel self settings set skills_auto_update false`). Turn this off with:
...
| `skills_auto_update` | boolean | Update the installed agent skills along with the CLI (needs `auto_update`) | `true`, `false` |
